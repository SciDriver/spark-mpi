...
class SocketAdapter {
 public:
  ...
  SocketAdapter() {}
  SocketAdapter(vector<boost::shared_ptr<SocketChannel> > * channels);

  vector<boost::shared_ptr<SocketChannel> > *channels;
...
class Socket {
 public:
	...
	write(const_cast<void*>(reinterpret_cast<const void *>
				(str.c_str())), str.size());
    }
