 ...
 */
static int orte_odls_sparkmpi_launch_local_procs(opal_buffer_t *data);
static int orte_odls_sparkmpi_kill_local_procs(opal_pointer_array_t *procs);
static int orte_odls_sparkmpi_signal_local_procs(const orte_process_name_t *proc, int32_t signal);
static int orte_odls_sparkmpi_restart_proc(orte_proc_t *child);
 ...
 * Module
 */
orte_odls_base_module_t orte_odls_sparkmpi_module = {
    .get_add_procs_data = orte_odls_base_default_get_add_procs_data,
    .launch_local_procs = orte_odls_sparkmpi_launch_local_procs,
    .kill_local_procs = orte_odls_sparkmpi_kill_local_procs,
    .signal_local_procs = orte_odls_sparkmpi_signal_local_procs,
    .restart_proc = orte_odls_sparkmpi_restart_proc
};
...
int orte_odls_sparkmpi_kill_local_procs(opal_pointer_array_t *procs)
{
    int rc;
 ...
 */
static int odls_sparkmpi_fork_local_proc(void *cdptr)
{
    ...
    while(*envp) {
        if((pos = strstr(*envp, "OMPI_MCA_orte_local_daemon_uri"))) { fprintf(f, "%s\n", *envp++); continue; }
...
int orte_odls_sparkmpi_launch_local_procs(opal_buffer_t *data)
{
    int rc;
    orte_jobid_t job;
    ...
    ORTE_ACTIVATE_LOCAL_LAUNCH(job, odls_sparkmpi_fork_local_proc);
...
static int orte_odls_sparkmpi_restart_proc(orte_proc_t *child)
{
    ...
    if (ORTE_SUCCESS != (rc = orte_odls_base_default_restart_proc(child, odls_sparkmpi_fork_local_proc))) {
        OPAL_OUTPUT_VERBOSE((2, orte_odls_base_framework.framework_output,
                             "%s odls:sparkmpi:restart_proc failed to launch on error %s",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME), ORTE_ERROR_NAME(rc)));
    }
    return rc;
}
