...
void RDMAAdapter::InternalThreadEntry() {
  ...
  while (!must_stop()) {
	...
	RDMABuffer* buff = channel->buffers_[id];
        received_.push(buff);
      } else {
...
RDMAChannel::RDMAChannel(const RDMAAdapter& adapter)
  ...
  // Create queue pair
  {
    ...
    memset(&attr, 0, sizeof(ibv_qp_init_attr));  // NOLINT(caffe/alt_fn)
  ...
  // Init queue pair
  {
    ...
    memset(&attr, 0, sizeof(ibv_qp_attr));  // NOLINT(caffe/alt_fn)
...
void RDMAChannel::Connect(const string& address) {
  ...
  char msg[250];
  sprintf(msg, "%04x:%06x:%06x", peer.lid, peer.qpn, peer.psn);
...
void RDMAChannel::recv() {
  ...
  memset(&wr, 0, sizeof(wr));  // NOLINT(caffe/alt_fn)
